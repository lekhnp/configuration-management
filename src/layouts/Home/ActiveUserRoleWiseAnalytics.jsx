...
const ActiveUserRoleWiseAnalytics = () => {
  ...
  const uniqueRoleName = useSelector(state => state.Role.data.list.map(obj => obj.roleName)).sort((a, b) => a.toLowerCase() > b.toLowerCase() ? 1 : -1);
        ...
        <Grid item xs={3} style={{ display: "flex" }}>
          <div style={{ flex: 1, display: "flex" }}>
            <CardContent className={styles.cardContent}>
              ...
              >
                <CountInsideCircular
                  value={`${Object.keys(userStats).length !== 0 ? userStats.active_users : 0}`}
                  size={40}
                  thickness={0}
                  valueTextVariant="h4"
                  ...
                  options={{
                    ...
                    plugins: {
                      datalabels: {
                        ...
                        formatter: (value, ctx) => {
                          const label =
                            parseFloat((value / 48) * 100).toFixed(0) + "%"; //ctx.chart.data.labels[ctx.dataIndex];
                          return label;
                        },
                      },
                    },
