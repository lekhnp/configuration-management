...
const ModuleUsesTrends = () => {
  ...
  const moduleUsageData = useSelector(state => state.Dashboard.moduleUsageData);
  const sortedList = moduleUsageData.length > 0 && moduleUsageData.filter(obj => obj.client_assignments > 0).sort((a, b) => (a.module_name.toLowerCase() > b.module_name.toLowerCase()) ? 1 : -1);
  const moduleName = sortedList.length > 0 && sortedList.map(obj => obj.module_name);
  const clientsAssigned = sortedList.length > 0 && sortedList.map(obj => obj.client_assignments);
  const totalFields = sortedList.length > 0 && sortedList.map(obj => obj.controls);
  const configurable = sortedList.length > 0 && sortedList.map(obj => obj.configurable);
