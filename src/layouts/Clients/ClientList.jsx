...
const ClientList = () => {
  ...
  const startIndex = useSelector((state) => state.Client.page.startIndex);
  ...
  const handleAssignManageClientDialog = (inputs) => {
    dispatch(addClientModules(inputs));
    // dispatch(saveClientInfo(clientSavedInfo, inputs));
    // setActiveTab((prevState) => prevState + 1);
  };
  ...
  useEffect(() => {
    ...
    dispatch(fetchClientsProfile(DEFAULT_START_INDEX, DEFAULT_PAGE_SIZE));
