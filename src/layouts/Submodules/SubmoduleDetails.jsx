...
import { handleSubmoduleDetailsError, NO_RECORDS_MESSAGE } from "../../utils/Messages";
...
const SubmoduleDetails = (props) => {
  ...
  const isSubmoduleDeleted = useSelector((state) => state.OOBSubmodule.isSubmoduleDeleted);
  ...
  const handleChange = (e) => {
    ...
    if (e.target.value === "") {
      ...
      dispatch(fetchOOBSubmodulesByOOBModuleId(moduleId, versionId, DEFAULT_START_INDEX, pageSize));
    }
  }
  ...
  const handleSearch = () => {
    if (searchText !== "") {
      dispatch({ type: SET_DEFAULT_STARTINDEX });
      dispatch(fetchOOBSubmodulesByOOBModuleId(moduleId, versionId, DEFAULT_START_INDEX, pageSize, searchText));
    }
  }
  ...
  useEffect(() => {
    if (isSubmoduleDeleted) {
      if (startIndex + 1 === totalElements && startIndex !== DEFAULT_START_INDEX)
        dispatch(fetchOOBSubmodulesByOOBModuleId(moduleId, versionId, startIndex - pageSize, pageSize, searchText));
      else
        dispatch(fetchOOBSubmodulesByOOBModuleId(moduleId, versionId, startIndex, pageSize, searchText));
    }
  }, [dispatch, isSubmoduleDeleted, moduleId, versionId, totalElements, startIndex, pageSize, searchText]);

  useEffect(() => {
    if(getApiError)
      setApiError(handleSubmoduleDetailsError(getApiError));
    else
      setApiError(false);
  },
    [getApiError]
  ...
  return (
    <>
      ...
      {apiError ? (
        ...
        <SubmoduleList openFrom={label} editable={editable} searchText={searchText} />
      ) : (
            <MatCard>
      ...
      {open && (
        <ManageSubmodule
          ...
          heading={"Add New " + label + " Component"}
