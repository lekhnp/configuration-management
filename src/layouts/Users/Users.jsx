...
const Users = () => {
  ...
  const startIndex = useSelector((state) => state.User.page.startIndex);
    ...
    actions: featuresAssigned.indexOf(VIEW_USER_PROFILE) !== -1 && {
      ...
      tooltipText:
        featuresAssigned.indexOf(UPDATE_USER_PROFILE) === -1
          ...
          : VIEW_UPDATE_PROFILE,
      action: (data) => {
        history.push(`/admin/user-profile/${data.email}`);
      },
    },
  };
  ...
  useEffect(() => {
    ...
    dispatch(fetchUsers(DEFAULT_START_INDEX, DEFAULT_PAGE_SIZE));
