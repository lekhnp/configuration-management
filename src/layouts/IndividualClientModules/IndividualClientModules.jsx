...
const IndividualClientModules = () => {
  ...
  const tableConfig = {
    ...
    actions: {
      ...
      action: (data) => {
        history.push(
          `/client/components/${label}/${clientId}/${data.id}/${data.versionId}/${data.currentVersion}`
        );
      },
    },
  };
  ...
  const handlePageChange = (start, size) => {
    if (label === "Global") {
        dispatch(
          fetchClientModule("global", clientId,loggedInUserData.user_type, start, size, searchText)
        );
    } else {
