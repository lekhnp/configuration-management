...
import AddMapping from "../ManageMapping/AddMapping";
import DataTable from "../FieldListTable";
...
const OobFieldMapping = (props) => {
  ...
  const { register, setValue, clearError } = useForm({
    mode: "onBlur",
  });
  ...
  const [inputs] = useState(defaultFormObj);
  ...
  const closeAddMappingDialog = useCallback(() => {
    setOpen(false);
  ...
  return (
    <>
      ...
      {open && <AddMapping handleClose={closeAddMappingDialog} open={open} />}
    </>
  );
};
