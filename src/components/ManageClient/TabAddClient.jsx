...
import { CLIENT_ALREADY_EXIST_MSG, CLIENT_NAME_MANDATORY_MSG, CODE_VERSION_MANDATORY_MSG,
    handleTabAddClientError, MAXIMUN_CHARACTER_ALLOWED_MSG, RELATIONSHIP_MANAGER_MANDATORY_MSG, VALID_NAME_MSG } from '../../utils/Messages';
    ...
    return (
        <form noValidate autoComplete="off" onSubmit={handleSubmit(() => handleNext(inputs))} id="addClientInfo">
            ...
            <Grid container className={styles.row}>
                ...
                <MaterialTextField label="Middle Name" id="middleName" name="middleName" variant="filled" value={middleName} onChange={handleChange} />
            </Grid>
            <Grid item xs={4} className={styles.col}>
                <MaterialTextField required label="Last Name" id="lastName" name="lastName" variant="filled" value={lastName} onChange={handleChange} />
            </Grid>
            <Grid item xs={6} className={styles.col}>
                <MaterialTextField required label="Email" id="email" name="email" variant="filled" />
            </Grid>
            <Grid item xs={6} className={styles.col}>
                <MaterialTextField label="Contact Number" id="contactno" name="contactno" variant="filled" />
            </Grid>
            <Grid item xs={4} className={styles.col}>
                <MaterialTextField label="Website" id="website" name="website" variant="filled" />
            </Grid>
            <Grid item xs={4} className={styles.col}>
                <MaterialTextField label="Organization" id="organization" name="organization" variant="filled" />
            </Grid>
            <Grid item xs={4} className={styles.col}>
                <MaterialTextField label="Designation" id="designation" name="designation" variant="filled" />
            </Grid>
            <Grid item xs={12} className={styles.col}>
                <MaterialTextField label="Address1" id="address1" name="address1" variant="filled" />
            </Grid>
            <Grid item xs={12} className={styles.col}>
                <MaterialTextField label="Address2" id="address2" name="address2" variant="filled" />
            </Grid>
            <Grid item xs={4} className={styles.col}>
                <MaterialTextField select label="State" id="state" name="state" variant="filled" onChange={handleChange}>
                {Object.keys(stateCityList).map((option, key) => (
            ...
            <Grid item xs={4} className={styles.col}>
                <MaterialTextField select label="City" id="city" name="city" variant="filled" disabled={city}>
                    {state !== '' && stateCityList[state].map((option, key) => (
                        <MenuItem key={option} value={option}>
                            {option}
                        </MenuItem>
                    ))}
            ...
            <Grid item xs={4} className={styles.col}>
                <MaterialTextField label="Zip Code" id="zip" name="zip" variant="filled" />
            </Grid>
            <Grid item xs={6} className={styles.col}>
                <MaterialTextField required select label="Role" id="role" name="role" variant="filled">
                    {role.map((option,key) => (
