...
const ClientEnvironment = (props) => {
  ...
  const handleSetEnvironment = (domainURL) => {
    let newElement = {};
    let Environment = {};
    newElement["clientName"] = props.details.clientName;
    Environment["domainUrl"] = domainURL;
    Environment["enable"] = true;
    Environment["environmentId"] = checked[checked.length - 1];
  ...
  const openConfirmDeleteDialog = (payload, envName) => {
    let messageObj = {
      ...
      primaryButtonAction: () => {
        dispatch(addClientEnvironment(payload));
      },
          ...
          >
            <Grid container className={styles.row}>
              {environmentList.map((env) => (
                <Grid key={env.id} item xs={4} className={styles.col}>
            ...
            <Grid item xs={12} className={styles.buttonCol}>
              ...
              {!isEditable && loggedInUserData.user_type !== "CLIENT" && featuresAssigned.indexOf(UPDATE_CLIENT_PROFILE) !== -1 && !props.details.isDeleted && (
