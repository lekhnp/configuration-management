...
//  const [activeModule, setActiveModule] = React.useState(null);
  ...
  const handleClientStatusClass = (status) => {
    if (status === "Invalid Tag Indicator") {
            ...
            {stableSort(props.rows, getComparator(order, orderBy)).map(
              (row, index) => (
                <StyledTableRow key={index}>
                  ...
                  <MatTableCell className={styles.tableRow}>
                     <Chip
                          label= {row.inValid?"Invalid file type":"Invalid Tag Indicator"}
                          className={handleClientStatusClass(
                            row.inValid?"Invalid file type":"Invalid Tag Indicator"
                          )}
                          color="primary"
                        />
                  </MatTableCell>
